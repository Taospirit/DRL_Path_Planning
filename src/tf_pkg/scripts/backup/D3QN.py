...
class DQN:
    ...
    def __init__(self):
        ...
        # Initial parameters
        self.Num_start_training, self.Num_training, self.Num_test = 0, 0, 20
        ...
        # 静态障碍下训练好的模型
        self.saved_networks_path = '/home/lintao/projects/DRL_Path_Planning/src/tf_pkg/scripts/saved_networks/'
        # self.load_path = '/home/lintao/projects/DRL_Path_Planning/src/tf_pkg/scripts/saved_networks/10_D3QN_PER_image_add_sensor_obstacle_world_30m_2_2019_06_02'

        self.step, self.score, self.episode = 1, 0, 0

        # date - hour - minute - second of training time
        self.date_time = str(datetime.date.today())

        # parameters for skipping and stacking
        self.Num_skipFrame, self.Num_stackFrame = 1, 4
    ...
    def xavier_initializer(self, shape):
        r'''
        get a random uniform value of 'shape' between -bound and bound
        '''
    ...
    def assign_network_to_target(self):  # unused
    ...
    def network(self):
        ...
        with tf.variable_scope('network'):
            ...
            cell = tf.nn.rnn_cell.LSTMCell(num_units=self.Num_cellState)
        ...
        rnn_out = rnn_out[-1] # 激光数据

        h_concat = tf.concat([h_pool3_flat, rnn_out], axis=1) # 图像数据和激光数据联合
        
        #TODO:
        # 为什么要分开两个? 为什么matmul后不加入非线性激活函数? 
        # 计算h_fc2_action的优势?
        ...
        with tf.variable_scope('target'):
            ...
            cell_target = tf.nn.rnn_cell.LSTMCell(num_units=self.Num_cellState)
    ...
    def init_sess(self):
        ...
        check_save = input('Load Model? (yes/no): ')

        if check_save == 'y':
            # Restore variables from disk.

            network_name = '20190601e'
            # network_name = '10_D3QN_PER_image_add_sensor_obstacle_world_30m_2_2019_06_02'

            save_dir_path = self.saved_networks_path + network_name + '/model.ckpt'
            print(save_dir_path)

            # model_path = tf.train.latest_checkpoint(save_dir_path)
            model_path = save_dir_path

            print('Start load model from ' + save_dir_path)
            saver.restore(sess, model_path)
            print("Model restored.")

            check_train = input('Inference or Training? (y=Inference / n=Training): ')
            if check_train == 'y':
...
# endregion
    ...
    # Initialize input
    def input_initialization(self, env_info):  # 出现error
        ...
        # state_stack = [[state], [state], [state], [state]]
    ...
    def resize_input(self, env_info, observation_set, state_set):
        ...
        del state_set[0] # 清理第一帧激光和自身数据

        observation = env_info[1]  # image
        observation_set.append(observation)
        # 使用观察组根据跳帧和堆叠帧的数量堆叠帧
        observation_stack = np.zeros(
            (self.img_size, self.img_size, self.Num_stackFrame))
        for stack_frame in range(self.Num_stackFrame):  # image 叠加4帧?
            observation_stack[:, :, stack_frame] = observation_set[-1 -
                                                                   (self.Num_skipFrame * stack_frame)]
        del observation_set[0]  # 删除第一帧图像
        observation_stack = np.uint8(observation_stack)
    ...
    def dist(self, p1_x, p1_y, p2_x, p2_y):
    ...
    def main(self):
        ...
        # 小车的起点 \ 终点 \ 起始转角
        start_x, start_y, goal_x, goal_y, randangle = [], [], [], [], []
        ...
        while(True):
            ...
            if self.dist(randposition[0][0], randposition[0][1], randposition[1][0], randposition[1][1]) > 20.0:
                break
        ...
        # randangle_temp = env.reset_env(start=[randposition[0][0], randposition[0][1]], goal=[randposition[1][0], randposition[1][1]])
        randangle_temp = env.reset_env(start=[start_x[-1], start_y[-1]], goal=[goal_x[-1], goal_y[-1]])
        ...
        start_x, start_y, goal_x, goal_y, randangle = [], [], [], [], []        # 全部清零?
        # endregion
        ...
        # env_info_array = np.array(env_info)
        # print ('===========================env_info================================')
        # print ('============= The shape of env_info is {}'.format(env_info_array.shape))
        # print ('------------- env_info[0]-laser & selfstate-shape is {}= {}'.format(np.array(env_info_array[0]).shape, env_info_array[0]))
        # print ('------------- env_info[1]-imgae- shape is {}= {}'.format(np.array(env_info_array[1]).shape, env_info_array[1]))
        # print ('------------- env_info[2]-terminal- is {}'.format(env_info_array[2]))
        # print ('------------- env_info[3]-reward- is {}'.format(env_info_array[3]))

        # print ('============= env is: {}'.format(env_info))
        # endregion
        ...
        while True:
            ...
            # 机器人执行动作
            v_cmd = ACTION_DICT[np.argmax(action)][0]
            omiga_cmd = ACTION_DICT[np.argmax(action)][1]
            env.step([v_cmd, omiga_cmd])
            ...
            self.next_observation_stack, self.observation_set, self.next_state_stack, self.state_set = self.resize_input(
                env_info, self.observation_set, self.state_set)  # 更新观察信息和状态信息
            terminal, reward = env_info[-2], env_info[-1]  # 获取terminal和reward
